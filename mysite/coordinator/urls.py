...
urlpatterns = [
    ...
    path('record/', ListView.as_view(queryset=Record.objects.all(), template_name='coordinator/record_list.html'),
         name='record_list'),
    path('record/<int:pk>', DetailView.as_view(model=Record, template_name='coordinator/generic_detail.html'), name='record_detail'),
    path('record/create', CreateView.as_view(model=Record, fields=['address', 'patient', 'patient_birthdate', 'temperature', 'doctor']), name='record_create'),
]